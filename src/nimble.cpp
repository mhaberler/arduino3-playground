...
static NimBLEAddress oat_address, envelope_address;
static ruuviAd_t ruuvi_report;

void setEnvMacAddress(std::string &mac) {
    LV_LOG_USER("setEnvMAC: %s\n", mac.c_str());

    // LV_LOG_USER("setEnvelopeMac: %lu\n", e->code);
    // SpiRamJsonDocument *jdoc = (SpiRamJsonDocument *)lv_event_get_user_data(e);
    // std::string mac((*jdoc)["payload"]["MAC"]);
    envelope_address = NimBLEAddress(mac);
}

void setOATMacAddress(std::string &mac) {
    LV_LOG_USER("setOATMac: %s\n", mac.c_str());
    // SpiRamJsonDocument *jdoc = (SpiRamJsonDocument *)lv_event_get_user_data(e);
    // std::string mac((*jdoc)["payload"]["MAC"]);
    oat_address  = NimBLEAddress(mac);
}
...
int getShortone(const byte *data, int index) {
...
void DecodeV5(const byte *data, ruuviAd_t *ra) {
    ra->ruuvi_format = 5;
    ra->temperature = (float)getShort(data, 3) * 0.005;
...
RUUVI_DECODED:
                        ...
                        if (advertisedDevice->getAddress() == envelope_address) {
                            lvgl_acquire();
                        ...
                        if (advertisedDevice->getAddress() == oat_address) {
                            lvgl_acquire();
